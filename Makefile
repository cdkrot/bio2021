...
SOLUTIONS: solutions/baseline.bin solutions/baseline-worse.bin solutions/thinice_brute.bin solutions/baseline-approx.bin solutions/baseline-mouse.bin
...
secret.zip: tests/*
	cd tests && zip -e -P LMQ8UrFb8fjbSD1AdutA ../secret.zip *.ans 20-mouse-simple-exact.txt

public.zip: tests/*
	rm -f public.7z
	cd tests && 7z a ../public_easy.7z 1*.txt 2*.txt 3*.txt 4*.txt 5*.txt 6*.txt
	cd tests && 7z a -mx=9 ../public_hard.7z 7*.txt 8*.txt 9*.txt
